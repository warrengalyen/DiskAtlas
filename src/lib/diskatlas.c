...
struct diskatlas_scan_result {
  ...
  /** duplicate_group_id dense in 1..dup_max_group_id; flat member index layout. */
  uint32_t dup_max_group_id;
  size_t *dup_group_off; /* len dup_max_group_id + 2 */
...
static int compute_duplicate_clusters(struct diskatlas_scan_result *r, uint32_t scan_flags);
...
static uint64_t fnv1a64_fold_w(const WCHAR *wide) {
...
static uint64_t basename_hash_ci_fold_utf8(const char *basename_utf8) {
  wchar_t stack[3072];
  const int cap = (int)(sizeof(stack) / sizeof(stack[0]));
  if (basename_utf8 == NULL || basename_utf8[0] == '\0') {
    return fnv1a64_fold_w(L"");
  }
  int n = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, basename_utf8, -1, stack, cap);
  if (n <= 0) {
    n = MultiByteToWideChar(CP_UTF8, 0, basename_utf8, -1, stack, cap);
  }
  if (n <= 0) {
    return 14695981039346656065ULL /* hash of sentinel */;
  }
  DWORD z = (DWORD)wcslen(stack);
  if (z != 0) {
    CharLowerBuffW(stack, z);
  }
  return fnv1a64_fold_w(stack);
}
...
static void ClearAllDuplicateAssignments(struct diskatlas_scan_result *r) {
  if (!r->nodes) {
    return;
  }
  for (size_t i = 0; i < r->node_count; i++) {
    r->nodes[i].duplicate_group_id = DISKATLAS_DUPLICATE_GROUP_NONE;
    r->nodes[i].reserved_u32 = 0;
  }
}

static bool NodeIsEligibleFileEntry(const struct diskatlas_scan_result *r, size_t i) {
  uint32_t kind = DISKATLAS_NODE_KIND_MASK & r->nodes[i].attributes;
  return kind == DISKATLAS_NODE_KIND_FILE;
}

static int DupBuildMemberTable(struct diskatlas_scan_result *r, uint32_t max_gid) {
  ...
  uint32_t *wr =
      (uint32_t *)calloc((size_t)max_gid + 1u, sizeof(uint32_t));
  if (!wr) {
    free(hist);
    free(off);
    free(mem);
...
static int compute_duplicate_clusters(struct diskatlas_scan_result *r, uint32_t scan_flags) {
  ...
  for (size_t i = 0; i < r->node_count; i++) {
    ...
    rec[wi].name_hash = basename_hash_ci_fold_utf8(bn);
  ...
  while (rs < nf) {
    ...
    while (
        re < nf &&
        rec[re].size_bytes == rec[rs].size_bytes &&
...
scan_done:
  finalize_paths(r);
  (void)compute_duplicate_clusters(r, r->options_copy.flags);
  MemoryBarrier();
...
DISKATLAS_API const size_t *diskatlas_dup_group_members(const scan_result_t *result,
  ...
  MemoryBarrier();
  if (group_id > r->dup_max_group_id || r->dup_group_off == NULL ||
      r->dup_group_mem == NULL) {
    return NULL;
  }
